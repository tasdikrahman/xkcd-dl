...
def dict_exists():
    if not os.path.isfile(xkcd_dict_location):
        print(
            'XKCD list not created! Run the following: \nxkcd-dl --update-db')
        return False
    return True
...
def download_one(xkcd_dict, xkcd_num):
    ...
    if xkcd_number in xkcd_dict:
        ...
        to_download_single = "{base}/{xkcd_num}/".format(
            base=BASE_URL, xkcd_num=xkcd_number)
        print(
...
def show_xkcd(num):
    ...
    path = '{current_directory}/xkcd_archive/{name}/'.format(
        current_directory=WORKING_DIRECTORY, name=num)
...
parser = argparse.ArgumentParser(
    prog='xkcd-dl',
    description='Run `xkcd-dl --update-db` if running for the first time.')
parser.add_argument(
    '-u', '--update-db', action='store_true', help='Update the database')
parser.add_argument(
    '-l',
    '--download-latest',
    action='store_true',
    help='Download most recent comic')
group = parser.add_mutually_exclusive_group()
group.add_argument(
    '-d',
    '--download',
    help='Download specified comic by number',
    type=int,
    metavar='XKCD_NUM')
group.add_argument(
    '-a', '--download-all', action='store_true', help='Download all comics')
parser.add_argument(
    '-r',
    '--download-range',
    nargs='*',
    help='Download specified range',
    type=int)
...
parser.add_argument(
    '-s',
    '--show',
    help='Show specified comic by number',
    type=int,
    metavar='XKCD_NUM')
